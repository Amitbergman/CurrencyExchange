...
import bitcoin_arbitrage.arbitrage.public_markets as public_markets
import bitcoin_arbitrage.arbitrage.observers as observers
import bitcoin_arbitrage.arbitrage.config as config
...
class Arbitrer(object):
    ...
    def init_markets(self, markets):
        ...
        for market_name in markets:
            try:
                # exec("import bitcoin_arbitrage.arbitrage.public_markets." + market_name.lower())
                # market = eval(
                #     "bitcoin_arbitrage.arbitrage.public_markets." + market_name.lower() + "." + market_name + "()"
                # )

                exec("import bitcoin_arbitrage.arbitrage.public_markets." + market_name.lower() + " as current_market")
                market = eval(
    ...
    def tick(self):
        ...
        self.first_ask = self.depths['BitstampUSD']['asks'][0]["price"]
        self.first_bid = self.depths['BitstampUSD']['bids'][0]["price"]
    ...
    def loop(self):
        while True:
            ...
            self.tickers()
            self.tick()
            print(f"first_ask = {self.first_ask}, first bid = {self.first_bid}")
